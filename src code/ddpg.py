...
class ReplayMemory:
    ...
    def sample(self, batch_size, device):
        ...
        ## TODO ##
        ind_list = np.random.randint(0,len(self.buffer), size=batch_size)
        state, action, reward, next_state, done=[],[],[],[],[]
        ...
        return torch.FloatTensor(state).to(device), torch.FloatTensor(action).to(device), torch.FloatTensor(reward).to(device), torch.FloatTensor(next_state).to(device), torch.FloatTensor(done).to(device)
...
class DDPG:
    def __init__(self, args):
        ...
        ## TODO ##
        self._actor_opt =torch.optim.Adam(self._actor_net.parameters(),args.lra)
        self._critic_opt =torch.optim.Adam(self._critic_net.parameters(),args.lrc)
    ...
    def select_action(self, state, noise=True):
        ...
        ## TODO ##
        with torch.no_grad():
            input_state=torch.from_numpy(state).to(self.device,dtype=torch.float32)
            self._actor_net.eval()
            act=self._actor_net(input_state).cpu().numpy()

            if noise:
                # add noise
                act+=self._action_noise.sample()
            # restrict the range [-1,1], let -1<=act<=1
            act=np.clip(act,-1,1)
            return act
    ...
    def _update_behavior_network(self, gamma):
        ...
        not_final= done==False
        with torch.no_grad():
            a_next =  self._target_actor_net(next_state)
            q_next =  self._target_critic_net(next_state,a_next) 
            q_target = reward+(gamma*q_next*not_final) # if final will become only reward
        ...
        actor_loss = -torch.mean(self._critic_net(state,action))
        # optimize actor
        self._actor_net.zero_grad()
        self._critic_net.zero_grad()
        actor_loss.backward()
    ...
    def _update_target_network(target_net, net, tau):
        ...
        for target, behavior in zip(target_net.parameters(), net.parameters()):
            ## TODO ##
            with torch.no_grad():
                target.copy_(behavior * tau + target * (1 - tau))
    ...
    for n_episode, seed in enumerate(seeds):
        ...
        while True:
            ...
            if done:
                ...
                print(f'Episode: {n_episode}\tTotal reward: {total_reward:.2f}')
